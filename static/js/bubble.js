document.addEventListener('DOMContentLoaded', () => {
            ...
            for (let j = 0; j < n - i - 1; j++) {
                ...
                if (tempArr[j] > tempArr[j + 1]) {
                    ...
                    steps.push({
                        ...
                        indices: [j, j + 1],
                        sorted: Array.from({length: i}, (_, k) => n - 1 - k),
                        message: `Swapping ${tempArr[j]} and ${tempArr[j + 1]}!`
                    });
                } else {
                    steps.push({
                        array: [...tempArr],
                        ...
                        indices: [j, j + 1],
                        sorted: Array.from({length: i}, (_, k) => n - 1 - k),
                        message: `No swap needed because it is already in order`
                    });
                }
            }
        }
    ...
    function updateBoxStates(data) {
        ...
        boxes.forEach((box, index) => {
            ...
            } else if (data.action === 'compare' && data.indices.includes(index)) {
                box.classList.add('comparing');
            } else if ((data.action === 'swap' || data.action === 'noswap') && data.indices.includes(index)) {
    ...
    async function animateStep(data) {
        ...
        if (data.action === 'compare' || data.action === 'noswap') {
            box1.classList.add('lifted');
            box2.classList.add('lifted');
            await sleep(speed * 0.4);
            box1.classList.remove('lifted');
            box2.classList.remove('lifted');
